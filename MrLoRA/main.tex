...
We use the standard GLUE training/validation splits. For each task, we fine-tune the teacher model with full fine-tuning (FFT) and our MR-LoRA; baseline results for standard LoRA are provided in Table~\ref{tab:baseline-lora}. For distillation, we freeze the student model and train only the MR-LoRA adapters using a weighted combination of task loss and knowledge-distillation loss (KL divergence). The student models are distilled variants of the same family: DistilBERT-base, DistilRoBERTa-base, and DeBERTa-v3-small.
...
\begin{itemize}
    ...
    \item \textbf{Other LoRA variants}: AdaLoRA, OLoRA, DoRA, RS-LoRA (see Table~\ref{tab:teacher-variants}).
    \item \textbf{Baseline methods}: Standard LoRA and KD-LoRA results are provided in Table~\ref{tab:baseline-lora}.
\end{itemize}
...
\subsubsection{Baseline LoRA and KD-LoRA Results}
Table~\ref{tab:baseline-lora} reports the per-task GLUE scores for the baseline methods: full fine-tuning (FFT), standard LoRA applied to the teacher (Teacher LoRA), and single-rank LoRA applied to the frozen student with knowledge distillation (KD-LoRA).
\begin{table}[ht]
\centering
\footnotesize
\setlength{\tabcolsep}{4.5pt}
\caption{Per-task GLUE scores for baseline methods. FFT denotes full fine-tuning of the teacher; Teacher LoRA applies standard low-rank adapters directly to the teacher; KD-LoRA refers to single-rank adapters applied to the frozen student with knowledge distillation. Abbreviations: BERT (BERT-base teacher / DistilBERT-base student), RoB (RoBERTa-base / DistilRoBERTa-base), DeB (DeBERTa-v3-base / DeBERTa-v3-small); T-LoRA: Teacher LoRA; KD-LoRA: single-rank adapter with knowledge distillation. Metrics as in Table~\ref{tab:glue-scores}.}
\label{tab:baseline-lora}
\begin{tabular}{lccccccccc}
\toprule
\multirow{2}{*}{Task} & \multicolumn{3}{c}{BERT} & \multicolumn{3}{c}{RoB} & \multicolumn{3}{c}{DeB} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7} \cmidrule(lr){8-10}
 & FFT & T-LoRA & KD-LoRA & FFT & T-LoRA & KD-LoRA & FFT & T-LoRA & KD-LoRA \\
\midrule
CoLA       & 56.2 & 51.5 & 39.7 & 59.3 & 50.6 & 40.6 & 65.8 & 62.3 & 45.4 \\
SST-2      & 92.0 & 90.8 & 88.9 & 93.3 & 93.5 & 91.0 & 95.7 & 95.0 & 93.6 \\
MRPC       & 87.9 & 81.2 & 77.1 & 90.5 & 80.4 & 76.9 & 90.7 & 80.6 & 74.8 \\
QQP        & 89.5 & 86.2 & 83.3 & 89.2 & 87.0 & 83.9 & 90.9 & 88.8 & 85.5 \\
STS-B      & 88.2 & 86.9 & 82.1 & 90.2 & 87.0 & 82.8 & 90.4 & 81.9 & 84.9 \\
QNLI       & 90.7 & 90.2 & 86.1 & 92.5 & 91.9 & 87.4 & 93.7 & 93.8 & 90.3 \\
RTE        & 62.1 & 59.1 & 57.7 & 67.6 & 61.0 & 55.0 & 79.8 & 52.7 & 52.7 \\
WNLI       & 48.4 & 56.3 & 50.4 & 51.2 & 55.9 & 52.5 & 55.4 & 56.3 & 53.2 \\
MNLI$_m$   & 83.3 & 82.0 & 77.3 & 87.3 & 86.5 & 79.7 & 89.3 & 90.1 & 85.4 \\
MNLI$_{mm}$& 83.6 & 82.8 & 78.7 & 87.1 & 86.6 & 80.5 & 89.6 & 90.1 & 85.5 \\
\midrule
Score     & 78.2 & 76.7 & 72.1 & 80.8 & 78.0 & 73.0 & 84.1 & 79.2 & 75.1 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Per‑task Performance of LoRA Variants (Teacher)}
Table~\ref{tab:teacher-per-task-pct} presents per‑task GLUE scores expressed as percentages of the teacher’s full fine‑tuning (FFT) performance for seven LoRA variants, averaged across BERT, RoBERTa, and DeBERTa model families.
...
\label{tab:teacher-per-task-pct}
\small
\begin{tabular}{lccccccccc}
\toprule
...
MR‑LoRA‑RS & 93.3\% & 99.9\% & 96.7\% & 97.9\% & -- & 100.3\% & 91.1\% & 109.4\% & 93.3\% \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Per‑task Performance of LoRA Variants (Student)}
Table~\ref{tab:student-per-task-pct} shows the corresponding percentages for student distillation. Per‑task data are available only for LoRA and MR‑LoRA; other variants display only the overall GLUE percentage.

\begin{table}[ht]
\centering
\caption{Per‑task GLUE scores (percentage of teacher FFT) for student LoRA variants (knowledge distillation), averaged across BERT, RoBERTa, and DeBERTa model families. Per‑task data are available only for LoRA and MR‑LoRA; other variants show only the overall GLUE percentage.}
\label{tab:student-per-task-pct}
\small
\begin{tabular}{lccccccccc}
\toprule
Method & CoLA & SST‑2 & MRPC & QQP & STS‑B & QNLI & RTE & WNLI & GLUE \\
...
Table~\ref{tab:efficiency} compares the resource requirements of each method (measured at rank 8). All LoRA variants reduce trainable parameters by approximately 99.5\% relative to FFT. Memory reduction (~82\%) and inference speedup (4.2--4.9$\times$) are measured only for LoRA and MR‑LoRA, as other variants lack efficiency profiling data.

\begin{table}[ht]
\centering
\caption{Efficiency metrics for LoRA variants. Parameter reduction is relative to full fine‑tuning (FFT). Memory reduction and inference speedup are averaged across BERT, RoBERTa, and DeBERTa model families (only available for LoRA and MR‑LoRA).}
...
\textbf{Efficiency--accuracy trade-off.} Tables~\ref{tab:teacher-variants} and~\ref{tab:student-variants} show that MR-LoRA achieves the best accuracy per parameter among LoRA variants. In teacher fine-tuning, MR-LoRA retains 89.9\% of FFT performance using only 0.56\% of FFT parameters, while in student distillation it retains 92.4\% of teacher performance with 0.56\% of parameters. The inference speedup (Table~\ref{tab:efficiency}) further confirms that MR-LoRA delivers the fastest inference without sacrificing much accuracy.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{figures/mrlora.pdf}
