...
With Singular Value Decomposition, any matrix $A \in \mathbb{R}^{d \times k}$ can be written as:

\begin{align}
A = \sum_{i=1}^{r} \sigma_i u_i v_i^\top \label{eq:svd}
\end{align}

where $\sigma_1 \geq \sigma_2 \geq \cdots > 0$ are non-zero singular values of $A$, $r = rank(A)$, and $u_i, v_i$ are left and right singular vectors. While Eq.~\ref{eq:svd} is exact, allocating a separate LoRA module to each term would not be wise. Sicne the importance of singular values decays exponentially, the first few terms of Eq.~\ref{eq:svd} will approximate $A$ quite well.
If we allocate the rank budget $R$ uniformly as $R$ rank-1 LoRA modules, the model would have to figure out the exponentially decay of importance in the coefficients, since all $R$ rank-1 matrices are equally \textit{complex} in term of rank. However, not all terms are equally \textit{important} and 

A Taylor's series approximates a complicated function by graduately adding exponentially more complicated terms
\begin{align}
f(x) = \sum_{n=0}^{\infty} \frac{f^{(n)}(a)}{n!} (x-a)^n \label{eq:taylor}
\end{align}

where the complexity of a term is encoded into the basis function $(x-a)^n / {n!}$.
Inpired by Eq.~\ref{eq:taylor}, we propose encoding the complexity of a term a prior as the rank of its basis matrix, with learnable coefficients to adjust the importance of a term.

Grouping them into blocks yields a more parameter-efficient approximation.
Specifically, we partition the rank-$R$ decomposition into $K$ blocks with exponentially decaying ranks $\{R, R/2, R/4, \ldots, R/2^{K-1}\}$, where each block $k$ contains matrices $B_k \in \mathbb{R}^{d \times r_k}$ and $A_k \in \mathbb{R}^{r_k \times k}$ with $r_k = R/2^{k-1}$.
