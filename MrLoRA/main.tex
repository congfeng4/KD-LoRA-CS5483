...
Drawing inspiration from Taylor's series and Singluar Value Decomposition, the swiftly shrinking weights offer fine-grained capacity within similar parameter budgets, while
...
OLoRA (\cite{buyukaz2024olora}) improves the LLMs training convergence with othonormal initialization of the low-rank matrices.
...
While traditional KD methods requires fully fine-tuning parameters of the student, KD-LoRA (\cite{pmlr-v262-kdlora}) takes a hybird approach that freezes the student weights and training LoRA adapters to approximate the teacher's outputs.
Combing the benefits from both worlds, it achieves performance comparable to fully finetuning and LoRA, while significantly reduced the resource usages.
However, the same expressiveness constraint of LoRA remains in the KD scenario, as the single rank matrix in each layer is often too coarse to account for the capacity discrepancies between teacher and student. 

We propose \textbf{M}ulti-\textbf{R}ank \textbf{LoRA}, a LoRA variant that allocates trainable parameter budgets as a series of exponentially decaying rank matrices per linear layer. 
Drawing on singular value decomposition (SVD), any matrix $A$ can be decomposed as $A = U\Sigma V^T = \sum_i \sigma_i u_i v_i^T$, a weighted sum of rank-one matrices, with singular values $\sigma_i$ as coefficients and orthogonal matrices $u_i v_i^T$ as bases.
This series form elegantly decomposes complex structure into simpler components, with dominant directions captured by early terms and fine details by later ones.
To avoid explicitly performing SVD,
we draw on the exponential approximation idea of Taylor's series and transform the SVD form by grouping adjacent terms into exponentially decaying rank blocks.
This yields a series of matrices with rapidly decreasing ranks, rendering the parameter count bounded while preserving the multi-rank flexibility.
...
For each teacher model, we select student models from the same family (e.g., BERT-base as teacher with BERT-small as student). All experiments are conducted on four NVIDIA GeForce RTX 4090 GPUs. 
