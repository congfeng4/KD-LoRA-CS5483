#!/usr/bin/env python3
"""
Create Table II for KD-LoRA paper comparing FFT baseline with different LoRA variants
for BERT model across GLUE tasks.
...
# Model family (BERT only for Table II)
MODEL_FAMILY = 'bert'
...
def collect_all_data(results_dir: str = 'results') -> pd.DataFrame:
    """Collect all data relevant for Table II."""
    all_records = []
...
def create_table_iia_dataframe(raw_df: pd.DataFrame) -> pd.DataFrame:
    """Create Table IIa DataFrame: FFT baseline + LoRA variants (LoRA-only strategy)."""
    # Filter for FFT and LoRA (type=2) only
    table_df = raw_df[raw_df['Strategy'].isin(['FFT', 'LoRA'])].copy()
...
def create_table_iib_dataframe(raw_df: pd.DataFrame) -> pd.DataFrame:
    """Create Table IIb DataFrame: FFT baseline + LoRA variants (KD-LoRA strategy)."""
    # Filter for FFT and KD-LoRA (type=1) only
    table_df = raw_df[raw_df['Strategy'].isin(['FFT', 'KD-LoRA'])].copy()
...
def plot_variant_comparison(table_iia: pd.DataFrame, table_iib: pd.DataFrame):
    """Generate bar plots comparing variants across strategies."""
    if table_iia.empty or table_iib.empty:
        print("Cannot generate comparison plots: missing data")
        return
    
    # Extract average rows
    if 'Average' in table_iia.index and 'Average' in table_iib.index:
        ...
        if plot_data:
            ...
            # Pivot for plotting
            pivot_df = plot_df.pivot(index='Variant', columns='Strategy', values='Average Score')
            
            x = np.arange(len(PEFT_VARIANTS))
            width = 0.35
            
            # Plot bars
            if 'LoRA' in pivot_df.columns:
                lora_values = [pivot_df.loc[v, 'LoRA'] if v in pivot_df.index and 'LoRA' in pivot_df.columns else np.nan for v in PEFT_VARIANTS]
                plt.bar(x - width/2, lora_values, width, label='LoRA', alpha=0.8)
            
            if 'KD-LoRA' in pivot_df.columns:
                kd_lora_values = [pivot_df.loc[v, 'KD-LoRA'] if v in pivot_df.index and 'KD-LoRA' in pivot_df.columns else np.nan for v in PEFT_VARIANTS]
                plt.bar(x + width/2, kd_lora_values, width, label='KD-LoRA', alpha=0.8)
            
            plt.xlabel('PEFT Variant')
            plt.ylabel('Average Metric Score')
            plt.title('Average Performance Across GLUE Tasks: LoRA vs KD-LoRA Variants')
            plt.xticks(x, PEFT_VARIANTS, rotation=45)
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig('variant_comparison.png', dpi=150)
            plt.close()
            print("Variant comparison plot saved to variant_comparison.png")

def main():
    print("=" * 80)
    print("KD-LoRA Table II Generator")
    print("Comparing FFT baseline with LoRA variants for BERT on GLUE tasks")
    print("=" * 80)
    ...
    print("\n2. Creating Table IIa (FFT + LoRA variants)...")
    table_iia = create_table_iia_dataframe(raw_df)
    ...
    if not table_iia.empty:
        ...
        print("\n" + "=" * 80)
        print("TABLE IIa: FFT Baseline vs LoRA Variants (LoRA-only strategy)")
        print("Rows: GLUE tasks (including MNLI matched/mismatched)")
        print("Columns: FFT baseline + 6 LoRA variants")
        print("=" * 80)
        print(table_iia.round(4))
        
        # Save to CSV
        csv_path = 'table_iia_results.csv'
        table_iia.to_csv(csv_path)
        print(f"\nTable IIa saved to {csv_path}")
        
        # Generate LaTeX
        latex_table = generate_latex_table(
            table_iia,
            'Performance of LoRA variants (LoRA-only strategy) for BERT on GLUE tasks',
            'tab:table_iia'
        )
        latex_path = 'table_iia_latex.tex'
        ...
        plot_heatmap(
            table_iia,
            'table_iia_heatmap.png',
            'Table IIa: FFT Baseline vs LoRA Variants (LoRA-only)'
        )
    else:
        print("No data for Table IIa!")
    
    # Step 3: Create Table IIb (KD-LoRA)
    print("\n3. Creating Table IIb (FFT + KD-LoRA variants)...")
    table_iib = create_table_iib_dataframe(raw_df)
    
    if not table_iib.empty:
        print("\n" + "=" * 80)
        print("TABLE IIb: FFT Baseline vs LoRA Variants (KD-LoRA strategy)")
        print("Rows: GLUE tasks (including MNLI matched/mismatched)")
        print("Columns: FFT baseline + 6 LoRA variants")
        print("=" * 80)
        ...
        # Save to CSV
        csv_path = 'table_iib_results.csv'
        table_iib.to_csv(csv_path)
        print(f"\nTable IIb saved to {csv_path}")
        
        # Generate LaTeX
        latex_table = generate_latex_table(
            table_iib,
            'Performance of LoRA variants (KD-LoRA strategy) for BERT on GLUE tasks',
            'tab:table_iib'
        )
        latex_path = 'table_iib_latex.tex'
        with open(latex_path, 'w') as f:
            f.write(latex_table)
        print(f"LaTeX table saved to {latex_path}")
        ...
        plot_heatmap(
            table_iib,
            'table_iib_heatmap.png',
            'Table IIb: FFT Baseline vs LoRA Variants (KD-LoRA)'
        )
    else:
        print("No data for Table IIb!")
    
    # Step 4: Generate comparison plots
    print("\n4. Generating comparison plots...")
    if not table_iia.empty and not table_iib.empty:
        plot_variant_comparison(table_iia, table_iib)
    ...
    print("\nExperiments by strategy and variant:")
    count_df = raw_df.groupby(['Strategy', 'PEFT Variant']).size().reset_index(name='Count')
    print(count_df.to_string(index=False))
    
    print("\nTasks with data:")
    for task in sorted(raw_df['Task'].unique()):
        task_df = raw_df[raw_df['Task'] == task]
        variants = sorted(task_df['PEFT Variant'].unique())
        strategies = sorted(task_df['Strategy'].unique())
        print(f"  {task}: {len(task_df)} experiments, strategies: {strategies}, variants: {variants}")
    
    print("\n" + "=" * 80)
    print("Table II generation complete!")
    print("=" * 80)
